...
int main() {
	...
	h.onMessage([&mpc](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
			...
			Eigen::VectorXd car_pt_x(ptsx.size());
			Eigen::VectorXd car_pt_y(ptsy.size());

			for (size_t i = 0; i < ptsx.size(); i++)
			{
				double x = ptsx[i] - px;
				double y = ptsy[i] - py;
				car_pt_x[i] = x * cos_psi - y * sin_psi;
				car_pt_y[i] = x * sin_psi + y * cos_psi;
			}


			auto coeffs = polyfit(car_pt_x, car_pt_y, 3);
			double cte = coeffs[0];
